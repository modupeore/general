	...
	foreach my $SNewFolder (@Subdirectory) {
		if ($SNewFolder =~ /^\w.*-(\d.*)_DP5.vcf$/){
		    ...
		    my $libidnew = $1; my $newrefgenome = "chicken";
                CHECKING();
		    unless (exists $Hashresults{$libidnew}){
...
#                    if ($libidnew > $aaa && $libidnew <= $bbb) {
                      ...
                      $parsedinput = "$input/$SubNewFolder"; `mkdir $parsedinput`;
...
#VARIANT_RESULTS
	...
	close (OUTDBVAR2);
      #VARIANT_SUMMARY
      $syntax = "update variants_summary set ANN_version = \"$vep_version\" where library_id = $libnumber";
      $sth = $dbh->prepare($syntax);
      $sth ->execute();
}
sub VEPVARIANT {
