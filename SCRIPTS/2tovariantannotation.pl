...
# #CREATING LOG FILES
my $std_out = '/home/modupeore17/.LOG/Vannotate1.log';
my $std_err = '/home/modupeore17/.LOG/Vannotate1.err';
...
$in1 = "/storage2/modupeore17";  ##the file path
...
foreach my $NewFolder (@Directory) { 
	...
	foreach my $SubNewFolder (@Subdirectory) {
		if ($SubNewFolder =~ /^\w.*_(\d.*)$/){
			my $libidnew = $1; my $newrefgenome = "fake";
                  CHECKING();
			#if($libidnew == 1126) {
...
sub VARIANTS{
	...
	#Making directory & testing the existence
      $Mfolder = "/storage2/modupeore17/CHICKENvariants/$libraryNO"; #`mkdir $Mfolder`;
