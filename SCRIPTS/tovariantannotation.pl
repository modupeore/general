...
# #CREATING LOG FILES
my $std_out = '/home/modupeore17/.LOG/Vannotate.log';
my $std_err = '/home/modupeore17/.LOG/Vannotate.err';
	...
	foreach my $SubNewFolder (@Subdirectory) {
		if ($SubNewFolder =~ /^\w.*_(\d.*)$/){
			...
			unless (exists $Hashresults{$libidnew}){
			  ...
			  VARIANTS($libidnew, "no", $newrefgenome, "no");
...
sub VARIANTS{
	...
	#Making directory & testing the existence
      $Mfolder = "/storage2/modupeore17/TOBE/TARS/$libraryNO"; #`mkdir $Mfolder`;
