...
my $jobid = "GECOadmin-".`date +%m-%d-%y_%T`;
...
my $STORAGEPATH = "/storage2/modupeore17/CHICKENvariants"; #files are stored here
my %parsablegenomes = ("alligator" => 2,"mouse" => 3, ); #genomes that work.
my %VEPparse = ("mouse" => 2, ); #for VEP
my $executemouse; #reworking the mouse assembly.
...
foreach (@Directory){
  if ($_ !~ /^\w*_\d*$/ && $_ =~ /^[a-zA-Z].*$/){push (@NewDirectory, $_);}
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# - - - - - - - - - - - - - - - - M A I N - - - - - - - - - - - - - - - - - - - - - -
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
#      } close ISOFORMS;
      ...
      unless(length($allgeninfo[$otherseq])<1){ #sequences 
        if ($refgenome =~ /mouse/) {
          $executemouse .= $allgeninfo[$seq].".gz ".$allgeninfo[$otherseq].".gz";
        }
        $sequences = ( ( split('\/', $allgeninfo[$seq]) ) [-1]).",". ( ( split('\/', $allgeninfo[$otherseq]) ) [-1]);
      } else {
        if ($refgenome =~ /mouse/) {
          $executemouse .= $allgeninfo[$seq].".gz";
        }
