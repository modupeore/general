...
# #CREATING LOG FILES
my $std_out = '/home/modupeore17/.LOG/Vannotateall.log';
my $std_err = '/home/modupeore17/.LOG/Vannotateall.err';
my $aaa = 500;
my $bbb = $aaa+100;
my $jobid = "VAnnotate-".`date +%m-%d-%y_%T`;
...
$in1 = "/storage2/modupeore17/TOBE";  ##the file path
...
#OPENING FOLDER
opendir(DIR,$in1) or die "General Folder \"$in1\" doesn't exist\n"; 
...
####DELETENOTDONE(); #not to be done for pipeline
foreach my $NewFolder (@Directory) { 
	my $input = $in1.$NewFolder;
	opendir(NEW,$input) or die "Folder \"$input\" doesn't exist\n";
	my @Subdirectory = readdir(NEW);
	close(NEW);
	foreach my $SNewFolder (@Subdirectory) {
		if ($SNewFolder =~ /^\w.*-(\d.*)_DP5.vcf$/){
              my $SubNewFolder = "library_$1";
		    my $libidnew = $1; my $newrefgenome = "chicken";
                CHECKING();
		    if (exists $Hashresults{$libidnew}){
                  if (exists $Birdresults{$libidnew}){
#                    if ($libidnew > $aaa && $libidnew <= $bbb) {
                      $syntax = "select ref_genome from frnak_metadata where library_id = $libidnew";
                      $sth = $dbh->prepare($syntax);$sth->execute or die "SQL Error: $DBI::errstr\n";
                      while ($row = $sth->fetchrow_array() ) {
                        $newrefgenome = $row;
                      }
                      $parsedinput = "$input/$SubNewFolder"; print "$parsedinput";
                      print "$libidnew what is this\n";
                      VARIANTS($libidnew, "no", $newrefgenome, "no", $parsedinput, $input);
...
#                    }
		...
		} else {print "no\t $SNewFolder\n";}	 
	}
}
# SUMMARYstmts(); not for pipeline as well
...
sub CHECKING {
	...
	$syntax = "select distinct(library_id) from variants_result";
	$sth = $dbh->prepare($syntax);
	$sth->execute or die "SQL Error: $DBI::errstr\n";
	my $number = 0;
	while ($row = $sth->fetchrow_array() ) {
	...
	$number = 0;
	while ($row = $sth->fetchrow_array() ) {
		$Birdresults{$row} = $number; $number++;
	}
}
sub PARSING {
	print "\n\tINSERTING TRANSCRIPTS INTO THE DATABASE : \t library_$_[0]\n\n";
	$lib_id = $_[0]; my $second = $_[1];
...
#  			} close ISOFORMS;
	...
	else {print "library_id dont match $numberz[0] == $lib_id\n";}
}
sub HTSEQ {
...
sub VARIANTS{
	print "\n\tWORKING ON VARIANT ANALYSIS\n\n";
 	my $libraryNO = "library-".$_[0]; my $bamfile = $_[1]; 
	my $REF= "$GENOMES/$_[2]/$_[2]".".fa";
	my $specie = $_[2];
	my $geneLIST = "$GENOMES/$_[2]/$_[2]".".txt";
 	my $DICT = "$GENOMES/$_[2]/$_[2]".".dict";
 	my $ANN = $_[3]; $ANN = "/home/modupeore17/.big_ten/alligator/amis_gbrowse_v1.2.gff";

	#Making directory & testing the existence
      $Mfolder = $_[4]; #"/storage2/modupeore17/TOBE/TARS/$libraryNO"; #`mkdir $Mfolder`;


	#ANNOTATIONS : running VEP
	print "this is the species $specie\n"; #Remove this Modupe
	if ($specie =~ /chicken/){
#        my $vepvcf = "perl $VEP -i $_[5]/".$libraryNO."_DP5.vcf --fork 24 --species $specie  --cache --vcf --merged --everything on --terms ensembl --output_file $Mfolder/".$libraryNO."_VEP.vcf";
#        `$vepvcf`;
#	#DATABASE INSERT
...
#		DBSNPDATVARIANTS($Mfolder."/".$libraryNO."_DP5.vcf", $Mfolder."/".$libraryNO."_DP5.vcf.output", $libraryNO);
		...
		print "Unidentified genome\t$mystderror\n"; "next";
	}
} 
sub FILTERING {
	my $input = $_[1];
...
sub DBVARIANTS{	
	...
	#OUTPUT file
	my $output = "$folder$information".".v1table"; print "$output\n";
...
#VARIANT_RESULTS
	while (<OUTDBVAR>){
        my ($a,$b,$c,$d,$e,$f,$g,$h) = split /\t/;
        if ($a =~ /^chr.*/) {
	    if ($g =~ /,/){
		my $first = split(",",$d);
		if (length $c == length $first){
		  $itvariants++; $itsnp++;
		}
		else {
		  $itvariants++; $itindel++;
      ...
      #VARIANT_SUMMARY
      print "XXX\t$libnumber\t$itvariants\t$itsnp\t$itindel\n";
      $syntax = "update variants_summary set total_VARIANTS = $itvariants, total_SNPS = $itsnp, total_INDELS = $itindel where library_id = $libnumber";
      $sth = $dbh->prepare($syntax);
      $sth ->execute();
}
sub VEPVARIANT {
          ...
          foreach my $FCR (0..$#finalchrsplit){
                ...
                if ($vepdetails[1] !~ /WITHIN_NON_CODING_GENE/){
                    ...
                    if (exists $Geneinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($Geneinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[4]){
                        ...
                        #exit;
                          ...
                          $Geneinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[4];
                    }
                    #TRANSCRIPT - 2
                    if (exists $Transcriptinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($Transcriptinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[6]){
                        ...
                        $Transcriptinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $temp1details;
                          ...
                          $Transcriptinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[6];
                    }
                    #CONSEQUENCE - 3
                    if (exists $Conqinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} ){
                      unless ($Conqinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[1]){
                        ...
                        #exit;
                          ...
                          $Conqinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[1];
                    }     
                    #VARIANT CLASS - 4
                    if (exists $Varclass{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($Varclass{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[20]){
                        ...
                        #exit;
                          ...
                          $Varclass{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[20];
                    }
                    #FEATURE TYPE - 5
                    if (exists $Featureinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                          ...
                          $Featureinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[5];
                    }
                    #PROTEIN POSITION - 6
                    if (exists $Proinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                          ...
                          $Proinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[14];
                    }
                    ...
                    if (exists $Prochangeinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($Prochangeinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[15]){
                        ...
                        $Prochangeinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $temp2details;
                          ...
                          $Prochangeinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[15];
                    }
                    #CODON CHANGE - 8
                    if (exists $Codoninfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($Codoninfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[16]){
                        ...
                        $Codoninfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $temp3details;
                          ...
                          $Codoninfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[16];
                    }
                    #dbSNP - 9
                    if (exists $dbSNPinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($dbSNPinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[17]){
                        ...
                        #exit;
                          ...
                          $dbSNPinfo{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[17];
                    }
                    #GENE name - 10
                    if (exists $GENEname{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($GENEname{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[3]){
                        ...
                        #exit;
                          ...
                          $GENEname{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[3];
                    }
                    #GENE type - 11
                    if (exists $GENEtype{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]}){
                      unless ($GENEtype{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} eq $vepdetails[7]){
                        ...
                        $GENEtype{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $temp4details;
                          ...
                          $GENEtype{$chrdetails[0]}{$chrdetails[1]}{$vepdetails[1]}{$vepdetails[14]}{$vepdetails[4]} = $vepdetails[7];
                    }
                }
          }
    }
  ...
  #my %entrez; my %ensembl; my %entrez2; my %ensembl2;
  #              
  #foreach my $aa (keys %Initialhash){
  #  foreach my $bb ( keys % { $Initialhash{$aa} }) {
  #    foreach my $cc ( keys % { $Initialhash{$aa}{$bb} }) {
  #      foreach my $dd ( keys % { $Initialhash{$aa}{$bb}{$cc} }) {
  #        foreach my $ee ( keys % { $Initialhash{$aa}{$bb}{$cc}{$dd} }) {
  #          my ($a,$b,$c,$d,$e,$f,$g,$h,$i,$j,$k,$l) = split('\|', $Initialhash{$aa}{$bb}{$cc}{$dd}{$ee});
  #          if ($c =~ /^ENSG.*/) {
  #            $ensembl{$aa}{$bb}{$cc}{$b}{$j}{$f} = "$aa$bb$cc$b$j$f";
  #            $ensembl2{$aa}{$bb}{$cc}{$b}{$j}{$f} = $c;
  #          }
  #          if (length($l) > 1) {
  #            $entrez{$aa}{$bb}{$cc}{$b}{$j}{$f} = "$aa$bb$cc$b$j$f";
  #            $entrez2{$aa}{$bb}{$cc}{$b}{$j}{$f} = $l;
  #          }
  #        }
  #      }
  #    }
  ...
  #        foreach my $ee ( keys % { $Initialhash{$aa}{$bb}{$cc}{$dd} }) {
  #          my ($a,$b,$c,$d,$e,$f,$g,$h,$i,$j,$k,$l) = split('\|', $Initialhash{$aa}{$bb}{$cc}{$dd}{$ee});
  #          if ($entrez{$aa}{$bb}{$cc}{$b}{$j}{$f} eq $ensembl{$aa}{$bb}{$cc}{$b}{$j}{$f}) {
  #            $VEPhash{$aa}{$bb}{$cc}{$dd}{$ee} = "$a|$b|$ensembl2{$aa}{$bb}{$cc}{$b}{$j}{$f}|$d|$e|$f|$g|$h|$i|$j|$k|$entrez2{$aa}{$bb}{$cc}{$b}{$j}{$f}";
  #          }
  #          else {
  #            $VEPhash{$aa}{$bb}{$cc}{$dd}{$ee} = "$a|$b|$c|$d|$e|$f|$g|$h|$i|$j|$k|$l";
  #          } 
  #        }
  #      }
  #    }
