...
$0 -- Extract comma genes information from specified library_id in the mysql database : transcriptatlas

=head1 SYNOPSIS

outputcommagenes.pl -1 <libraryid> -2 <output> [--help] [--manual]
...
foreach my $genename (sort keys %FPKM){
	if ($genename =~ /^[0-9a-zA-Z]/){
		if ($genename =~ /\,/){
			my @eachgenename = split("\,", $genename);
			...
			foreach my $newgene (@eachgenename){
				my ($newrealstart,$newrealstop) = split('\|',$REALPOST{$newgene},2);
				print OUT $newgene."\t".$CHROM{$newgene}."\:".$newrealstart."\-".$newrealstop."\t";		
				foreach my $lib (0..$#headers-1){ 
                                	if (exists $FPKM{$newgene}{$headers[$lib]}){
                                        	print OUT "$FPKM{$newgene}{$headers[$lib]}\t";
                                	}
                                	else {
                                        	print OUT "0\t";
                                	}
                        	}
                        	if (exists $FPKM{$newgene}{$headers[$#headers]}){
                                	print OUT "$FPKM{$newgene}{$headers[$#headers]}\n";
                        	}
                        	else {
                                	print OUT "0\n";
                        	}
