      ...
      <div id="Gene_Name">
        ...
        <?php
          if (!empty($_POST['gselect'])) {
            echo '<input type="text" name="gselect" value="' . $_POST["gselect"] . '"/>';
          } else {
            echo '<input type="text" name="gselect" placeholder="Enter Gene Name" />';
          }
        ?>
        </p><br>
...
<?php
  if ( (isset($_POST['vcgreveal']) || isset($_POST['vcareveal']) || isset($_POST['vcmreveal']))
    ...
    $pquery = 'perl '.$base_path.'/SQLscripts/outputvariantinfo.pl -i "chrom" -c '.$_POST['chrompos'].' -b '.$_POST['chromposbegin'].' -e '.$_POST['chromposend'].' -s '.$_POST['species'].'';
  ...
  elseif (isset($_POST['vgreveal']) && !empty($_POST['gselect'])) {
    $pquery = 'perl '.$base_path.'/SQLscripts/outputvariantinfo.pl -i "gene" -g '.$_POST['gselect'].' -s '.$_POST['species'].'';
    $rquery = shell_exec($pquery);
    if (count(explode ("\n", $rquery)) <= 13){
      echo '<center>No results were found with your search criteria.<center>';
    } else {
